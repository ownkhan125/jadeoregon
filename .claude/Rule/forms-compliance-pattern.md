...
2. **A leading `1` in the cleaned digit stream is stripped
   unconditionally** — not only when the input "looks like" a
   country-coded number. The formatter re-receives its own `+1 (…`
   output on every keystroke; if the strip is gated on digit-length
   (e.g. "only strip when ≥ 11 digits"), every mid-typing keystroke
   shifts digits one position to the right (`'+1 (4'` → user types
   `'8'` → browser returns `'+1 (48'` → digits `'148'` → formatter
   re-emits `'+1 (148'`). Per NANP, US area codes never start with
   `1`, so a leading `1` in cleaned digits is always either the
   country code or our re-fed prefix. We own the country code:
   `+1 (555) 555-0100`, `15555550100`, and `+15555550100` all
...
// IMPORTANT: the formatter is re-fed its own output on every keystroke,
// so the live-typing rows below are the ones the implementation must
// pass — not just the terminal-state paste cases above them.

// — Terminal-state values (paste, fully-typed)
...
// — Live-typing state (formatter re-fed its own output). These rows
//   exercise the unconditional leading-`1` strip from §2 rule 2. Skip
//   them and the field drifts one digit right on every keystroke.
formatPhoneInput('+1 (4')             → '+1 (4'   // first digit typed
formatPhoneInput('+1 (48')            → '+1 (48'  // NOT '+1 (148'
formatPhoneInput('+1 (1')             → '+1 ('    // NANP: no area code starts with 1
formatPhoneInput('+1 (1234567890')    → '+1 (123) 456-7890'
